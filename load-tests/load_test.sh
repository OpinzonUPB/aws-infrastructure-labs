...
# el resultado como una fila nueva en load-tests/results/results.csv.
#
# Uso:
#   ./load-tests/load_test.sh <usuarios_concurrentes> [duracion_segundos] [endpoint]
#
# Ejemplos:
#   ./load-tests/load_test.sh 10
#   ./load-tests/load_test.sh 25
#   ./load-tests/load_test.sh 50 30 /compute
#
...
USERS="${1:?Uso: ./load-tests/load_test.sh <usuarios_concurrentes> [duracion_segundos] [endpoint]}"
DURATION="${2:-30}"
ENDPOINT="${3:-/compute}"

CONTAINER="sizing-app"
ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
RESULTS_DIR="$ROOT_DIR/load-tests/results"
...
#    así no hace falta instalar nada más en la instancia EC2.
...
# 5. Registrar el resultado combinado en load-tests/results/results.csv.
python3 "$ROOT_DIR/load-tests/record_result.py" \
